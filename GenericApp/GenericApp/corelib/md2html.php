...
define("actionType_htmlentities", 12);
...
class md2html
{
            ...
            elseif(preg_match('/^\s*(?<type>\w+)\s*("(?<find>(""|[^"])+)")?\s*(("(?<replace>([^"]|\\\\")+)")|(?<label>\w+))?/', $rule, $match))
            {
                ...
                switch($match['type'])
                {
                    ...
                    case "restoreAll":
                        ...
                        break;
                    case "restore":
                        if(strlen($match['label']))
                            $this->rules[] = array('type'=>actionType_restore, 'label'=>$match['label']);
                        break;
                    case 'htmlentities':
                        if(isset($match['label']))
                            $this->rules[] = array('type'=>actionType_htmlentities, 'rx'=>$this->sort_regex($match['find']), 'label'=>$match['label']);
                        else
                            $this->rules[] = array('type'=>actionType_htmlentities, 'rx'=>$this->sort_regex($match['find']), 'label'=>false);
                        break;
                    case 'deleteline':
        ...
        foreach($this->rules as $rule)
        {
            ...
            switch($rule['type'])
            {
                ...
                case actionType_prevline:
                    ...
                    break;
                case actionType_htmlentities:
                    if($rule['label'])
                    {
                        foreach($this->savedBlocks as &$block)
                        {
                            if($block['type'] == $rule['label'])
                            {
                                 if(preg_match($rule['rx'], $block['block'], $match, PREG_OFFSET_CAPTURE))
                                {
                                    if(isset($match[1]))
                                    {
                                        $replace = htmlentities($match[1][0]);
                                        $block['block'] = substr_replace($block['block'], $replace, $match[1][1], strlen($match[1][0]));
                                    }
                                }
                            }
                        }
    ...
    function saveBlock($block, $blockTypeName)
    {
        ...
        $this->savedBlocks[] = array('type'=>$blockTypeName, 'block'=>$block);
    ...
    function restoreBlocks($blockTypeName)
    {
        ...
        {
            $savedIdx = intval($m[1]);
            $this->textBeingProcessed = str_replace($m[0], $this->savedBlocks[$savedIdx]['block'], $this->textBeingProcessed);
        }
    }
